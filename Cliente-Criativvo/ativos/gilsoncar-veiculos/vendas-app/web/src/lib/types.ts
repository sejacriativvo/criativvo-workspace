...
export type Store = 'ibitinga' | 'borborema';
...
export type Vehicle = {
  ...
  max_discount: number | null; // desconto definido pelo admin (R$); null = sem margem
  level: Level;
};
...
export type CarRow = {
  ...
  combustivel: string | null;
  price: number;
  cost: number | null;                 // opcional, só admin vê (lucro)
  negotiation_discount: number | null; // desconto máx (R$) que o vendedor pode dar
  opcionais: string[];
  img: string | null;
  in_prep: boolean;
  status: 'available' | 'sold';
  ...
  shopify_product_id: string | null;
  created_at: string;
  acquired_at: string | null;          // quando a GilsonCar comprou (base do giro)
...
export type Client = {
  ...
  payment: string | null;
  store: Store | null;           // loja da venda (Ibitinga/Borborema) — alimenta o relatório
...
export type Settings = {
  ...
  monthly_goal: number;
  goal_ibitinga: number;       // meta de vendas por loja
  goal_borborema: number;
  sales_ibitinga: number;      // contador manual de vendas do mês (+/- nas Metas)
  sales_borborema: number;
