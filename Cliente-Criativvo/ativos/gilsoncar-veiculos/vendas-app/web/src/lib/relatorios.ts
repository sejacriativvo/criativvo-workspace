...
// Totais por mês considerando a loja (ou "rede").
export function totalsFor(
  ...
  if (store === 'rede') {
    return sumTotals(
      monthTotals(rows, 'ibitinga', year, month),
      monthTotals(rows, 'borborema', year, month),
    );
  }
  return monthTotals(rows, store, year, month);
}
...
// Na "rede", soma as duas lojas por semana (mesmo período).
export function weeklyBreakdown(
  ...
  const stores: Store[] = store === 'rede' ? ['ibitinga', 'borborema'] : [store];
...
export function buildSalesMap(
  sales: { store: Store | null; sold_on: string | null }[],
): CrmMap {
  const map: CrmMap = {};
  for (const s of sales) {
    if (!s.store || !s.sold_on) continue;
...
// Meses que têm algum dado pra uma loja (pra navegação e gráfico anual).
export function monthsWithData(rows: AdReport[], store: Store | 'rede'): { year: number; month: number }[] {
