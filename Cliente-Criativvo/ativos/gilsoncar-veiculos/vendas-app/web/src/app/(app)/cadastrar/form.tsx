    ...
    <form action={formAction} className="space-y-4 bg-white rounded-2xl border border-neutral-200 p-5">
      ...
      <label className="block">
        <span className="text-sm font-semibold text-neutral-800">
          Margem de negociação (desconto máximo em R$)
        </span>
        <span className="block text-xs text-neutral-500 mb-1">
          É isso que o vendedor vê. Ex: 3000 = pode dar até R$ 3.000 de desconto.
        </span>
        <input
          ...
          step="100"
          required
          defaultValue={car.negotiation_discount ?? ''}
          placeholder="3000"
      ...
      <label className="block border-t border-neutral-100 pt-4">
        <span className="text-sm font-medium text-neutral-700">Custo (opcional, só você vê)</span>
        <span className="block text-xs text-neutral-500 mb-1">
          Preencha pra acompanhar lucro real. O vendedor nunca vê isso.
        </span>
        <input
          name="cost"
          type="number"
      ...
      <label className="block border-t border-neutral-100 pt-4">
        ...
        <span className="block text-xs text-neutral-500 mb-1">
          Quando a GilsonCar comprou esse carro. É a partir daqui que conta o tempo no estoque (giro).
        </span>
        <input
          name="acquired_at"
          type="date"
      ...
      >
        {pending ? 'Salvando...' : 'Salvar margem'}
      </button>
    </form>
  );
}
