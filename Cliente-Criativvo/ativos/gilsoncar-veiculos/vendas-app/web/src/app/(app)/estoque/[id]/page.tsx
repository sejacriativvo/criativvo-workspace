...
export default async function CarDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const m = car ? adminMetrics(car.price, car.cost, car.negotiation_discount) : null;
          ...
          {isAdmin && car && m && (
            <div className={`mt-4 rounded-2xl border p-4 ${vehicle.level === 'pending' ? 'border-amber-300 bg-amber-50' : 'border-neutral-200'}`}>
                ...
                <>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      ...
                      <div className="font-bold text-neutral-900">{brl(car.negotiation_discount ?? 0)}</div>
                    </div>
                    <div>
              ...
              <Link
                ...
                className={`mt-3 block text-center text-sm rounded-lg py-2 font-semibold ${
                  vehicle.level === 'pending'
