...
export default async function DashboardPage() {
  ...
  const vendasMes = (salesRows as { id: string }[] | null)?.length ?? 0;
  ...
  const estoqueComp = [
    { label: 'Com margem', value: available.filter((c) => !c.in_prep && c.cost != null).length, color: '#10b981' },
    { label: 'Sem margem', value: available.filter((c) => !c.in_prep && c.cost == null).length, color: '#f59e0b' },
