...
// Pop-up que lembra o admin de preencher o que falta (1x por sessão).
export function AdminReminder({
  ...
  const total = semMargem + semCusto;

  useEffect(() => {
    if (total === 0) return;
    try {
      if (sessionStorage.getItem('bm:reminder') === '1') return;
    } catch {}
    setOpen(true);
  }, [total]);
  ...
  function close() {
    try {
      sessionStorage.setItem('bm:reminder', '1');
    } catch {}
    setOpen(false);
  }
