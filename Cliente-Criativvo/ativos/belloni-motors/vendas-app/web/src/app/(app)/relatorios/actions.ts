...
// Loja única — constante usada nos upserts.
const STORE = 'belloni';
...
// Salva as semanas de um mês. Tráfego (Alisson) só grava os azuis
...
export async function saveWeeks(payload: {
  ...
  // Cada papel grava SÓ a sua parte.
...
// carros vendidos e o lucro do mês.
...
// Edição da VENDA direto na tabela do relatório (admin).
export async function setReportSales(payload: {
  ...
  if (isLiveMonth(payload.year, payload.month) && payload.granularity === 'week') {
    ...
    if (tgt > cur) {
      const novas = Array.from({ length: tgt - cur }, () => ({ sold_on: first }));
