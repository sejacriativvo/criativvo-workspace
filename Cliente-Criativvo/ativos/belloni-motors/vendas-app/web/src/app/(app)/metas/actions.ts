...
// Reconcilia o LOG de vendas (tabela sales) pro total do mês = `target`.
...
export async function setSaleCount(target: number) {
  await requireAdmin();
  ...
  if (tgt > cur) {
    const novas = Array.from({ length: tgt - cur }, () => ({ sold_on: today }));
...
// Salva a meta de vendas mensal.
export async function saveGoals(_prev: unknown, formData: FormData) {
  await requireAdmin();
  const goal = Number(formData.get('monthly_goal') || 0);
  const supabase = await createClient();
