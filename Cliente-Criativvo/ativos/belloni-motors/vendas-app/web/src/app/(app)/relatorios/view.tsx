...
import type { AdReport, Role } from '@/lib/types';
import {
  MESES,
  MES_ABREV,
  totalsFor,
...
const STORE_NAME = 'Belloni Motors';
const FEE = 4750;
...
export function RelatoriosView({ rows, crm, role }: { rows: AdReport[]; crm: CrmMap; role: Role }) {
  ...
  const t = totalsFor(displayRows, 'belloni', ym.y, ym.m);
  const tableRows: { label: string; total: Totals; gran?: 'week' | 'month'; pidx?: number; plabel?: string | null }[] =
    showHistorico && t
      ...
      : weekTableFor(displayRows, 'belloni', ym.y, ym.m).map((r, i) => ({ ...r, gran: 'week', pidx: i, plabel: r.label }));
  const monthTotal = tableRows.reduce<Totals | null>((acc, r) => sumTotals(acc, r.total), null);

  const prev = totalsFor(displayRows, 'belloni', ym.y - 1, ym.m); // mesmo mês, ano anterior
  ...
  const monthTable = yearBreakdown(displayRows, 'belloni', ym.y).filter((r) => r.total);

  // Lançamento. Sempre 4 semanas, mescladas com o que existe.
  const storeWeekRows = rows.filter((r) => r.year === ym.y && r.month === ym.m && r.granularity === 'week');
  const storeMonthly = rows.find((r) => r.year === ym.y && r.month === ym.m && r.granularity === 'month');
  const entryHistorico = storeWeekRows.length === 0 && !!storeMonthly;

  const initialWeeks = useMemo(() => {
    if (entryHistorico) return [];
  ...
  }, [rows, ym.y, ym.m]); // eslint-disable-line react-hooks/exhaustive-deps

  // Contagem do CRM por semana (mostra como referência no lançamento do admin).
  const entryCrm: Record<number, number> = {};
  for (let i = 0; i < 5; i++) {
    const k = `belloni-${ym.y}-${ym.m}-${i}`;
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-3 mb-1">
        <div>
          ...
          <p className="text-xs text-neutral-400">Tráfego pago — {STORE_NAME}</p>
        </div>
        <MetaSyncButton />
      </div>
      ...
      <div className="mt-3 bg-white rounded-2xl border border-neutral-200 p-4">
        ...
        <ResultsTable
          ...
          editable={
            role === 'admin'
              ? { year: ym.y, month: ym.m, onSaved: () => { flash('Venda atualizada'); router.refresh(); } }
              : undefined
          }
        />
      </div>
      ...
      <div className="mt-5">
        <h2 className="text-sm font-semibold text-neutral-700 mb-2">Lançamento — {STORE_NAME}</h2>
        {entryHistorico ? (
          role === 'admin' ? (
            <MonthHistoryEntry
              key={`mh-${ym.y}-${ym.m}`}
...
function ResultsTable({
  ...
  totalLabel?: string;
  highlight?: string;
  editable?: { year: number; month: number; onSaved: () => void };
}) {
...
function MonthHistoryEntry({
  ...
  function save() {
    ...
    start(async () => {
      ...
      const r = await saveMonthHistory({ year, month, vendidos, lucro: toNum(l) });
      if (r?.error) setErr(r.error);
      else onSaved();
    });
  }
