...
//
// Conta de anúncio única da Belloni Motors. Só mexe nos campos azuis;
// vendidos/lucro (verdes) ficam intactos.
// Fail-closed: sem token/conta configurados, não roda.
...
// Conta de anúncio da Belloni Motors (só o número, com ou sem o prefixo 'act_').
type Account = { store: string; accountId: string; token: string };

function accountMap(): Account[] {
  const token = process.env.META_ACCESS_TOKEN?.trim();
  const acc = process.env.META_AD_ACCOUNT_BELLONI?.trim();
  if (acc && token) return [{ store: 'belloni', accountId: acc.replace(/^act_/, ''), token }];
  return [];
}
...
export type MetaSyncResult = {
  ok: boolean;
  error?: string;
  stores?: { store: string; investimento: number; conversas: number; weeks: number }[];
};
